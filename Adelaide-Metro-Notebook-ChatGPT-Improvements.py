...
Version 6.7 - Fixed schema, robust retries, configurable cleanup, Jupyter-safe CLI, auto-README
...
1. REQUIREMENTS:
   pip install arcgis gtfs-realtime-bindings

2. AUTHENTICATION:
   - Run in ArcGIS Online Notebook (GIS("home") works automatically)
   - Or locally with ArcGIS Pro / ArcGIS API login

3. CONFIGURATION:
   Edit at top of script:
       FEATURE_LAYER_NAME = "Adelaide_Metro_Vehicles"
       KEEP_N = 3   # how many timestamped layers to keep
...
5. AUTO-README:
   First run will create README.md in the script directory (if missing).
------------------------------------------------------------
"""
...
KEEP_N = 3   # number of timestamped services to keep
    ...
    for entity in feed.entity:
        if entity.HasField("vehicle"):
            ...
            if vp.HasField("vehicle"):
                v["vehicle_id"], v["vehicle_label"], v["license_plate"] = vp.vehicle.id or None, vp.vehicle.label or None, vp.vehicle.license_plate or None
            if vp.HasField("trip"):
                td = vp.trip
                v["trip_id"], v["route_id"] = td.trip_id or None, td.route_id or None
                try: v["direction_id"] = td.direction_id if td.HasField("direction_id") else None
                except Exception: v["direction_id"] = getattr(td, "direction_id", None)
                v["start_time"], v["start_date"] = td.start_time or None, td.start_date or None
            if vp.HasField("position"):
                pos = vp.position
                v["latitude"], v["longitude"], v["bearing"], v["speed"] = getattr(pos,"latitude",None), getattr(pos,"longitude",None), getattr(pos,"bearing",None), getattr(pos,"speed",None)
...
def validate_and_filter_positions(vehicles: List[Dict]) -> List[Dict]:
    ...
    for v in vehicles:
        ...
        if lat and lon and lat != 0 and lon != 0:
            if -36.5 <= lat <= -33.5 and 137.5 <= lon <= 140.5: valid.append(v)
...
def prefer_feature_service(items):
    fs=[i for i in items if i.type=="Feature Service"]; fl=[i for i in items if i.type=="Feature Layer"]
    ...
    try:
        ...
        for v in vehicles:
            if v.get("latitude") and v.get("longitude"):
                attrs = {
                    "VehicleID": v.get("vehicle_id"), "VehicleLabel": v.get("vehicle_label"),
                    "LicensePlate": v.get("license_plate"), "TripID": v.get("trip_id"),
                    "RouteID": v.get("route_id"), "DirectionID": v.get("direction_id"),
                    "StartTime": v.get("start_time"), "StartDate": v.get("start_date"),
                    "Bearing": v.get("bearing"), "Speed": v.get("speed"),
        ...
        for batch in chunk(features, MAX_ADD_PER_REQUEST): fl.edit_features(adds=batch)
        print(f"✓ Updated {len(features)} features."); return True
    except Exception as e: print(f"Update error: {e}"); print(traceback.format_exc()); return False
...
def create_feature_layer_with_schema(gis: GIS, base_name: str):
    ...
    layer_def={"layers":[{"name":unique,"type":"Feature Layer","geometryType":"esriGeometryPoint","fields":fields,
                          "objectIdField":"OBJECTID","extent":{"xmin":137.5,"ymin":-36.5,"xmax":140.5,"ymax":-33.5,"spatialReference":{"wkid":4326}},
...
### Vehicle Position Fetcher (v6.7)

Fetches Adelaide Metro vehicle positions and publishes them to ArcGIS Online.
